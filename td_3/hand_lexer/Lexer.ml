...
let get_lexeme (automaton : Automaton.t) (string : string) (start_pos : int) =
  let rec step current_pos current_state last_pos last_state =
    let last_pos, last_state = 
      if Automaton.is_final automaton current_state then (current_pos, Some current_state) else (last_pos, last_state)
    in
    if current_pos = String.length string then (last_pos, last_state) else
    match Automaton.get_next_state automaton (current_state, string.[current_pos]) with 
    |None -> (last_pos, last_state)
    |Some s  -> step (current_pos + 1) s last_pos last_state 
    in
